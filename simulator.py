...
class Simulator:
    ...
    def check_active(self, index: int):
        ...
        if (
            ...
            return False
        # If the movie does not end, the agent continues shopping the movie.
        if agent.event.action_type == "shopping":
            self.round_msg.append(
                Message(
                    agent_id=agent.id,
                    action="BUY",
                    content=f"{agent.name} is purchasing goods.",
                )
            )
            return False
    ...
    def one_step(self, agent_id):
        ...
        if "SHOPPING" in choice:
            self.logger.info(f"{name} enters the shopping system.")
            message.append(
                Message(
                    agent_id=agent_id,
                    action="SHOPPING",
                    content=f"{name} enters the shopping system.",
                )
            )
            self.round_msg.append(
                Message(
            ...
            while not leave:
                ...
                if "BUY" in choice and (agent.event.action_type == "idle" or agent.event.action_type == "posting"):
                    ...
                    if len(item_descriptions) == 0:
                        self.logger.info(f"{name} leaves the shopping system.")
                        message.append(
                            Message(
                                agent_id=agent_id,
                                action="SHOPPING",
                                content=f"{name} leaves the shopping system.",
                            )
                        )
                        self.round_msg.append(
                            Message(
                    ...
                    for i in range(len(item_names)):
                        observation = f"{name} has just finished purchasing {item_names[i]};;{item_descriptions[i]}."
                    ...
                    if (page + 1) * self.recsys.page_size < len(rec_items):
                        ...
                        message.append(
                            Message(
                                agent_id=agent_id,
                                action="SHOPPING",
                                content=f"No more items. {name} leaves the shopping system.",
                            )
                        )
                        self.round_msg.append(
                            Message(
                    ...
                    observation = f"{name} is checking the product details."
                    
                    item_details = self.data.get_item_detail_by_name(
                        item_names
                    )
                    ...
                    if(len(item_details)==0):
                        ...
                        message.append(
                            Message(
                                agent_id=agent_id,
                                action="SHOPPING",
                                content=f"{name} do nothing and continue browsing the shopping system.",
                            )
                        )
                        self.round_msg.append(
                            Message(
            ...
            if "CHAT" in choice:
                if len(contacts) == 0:
                    self.logger.info(f"{name} has no acquaintance and {name} leaves the social media.")
                    message.append(
                        Message(
                            agent_id=agent_id,
                            action="SOCIAL",
                            content=f"{name} has no acquaintance and {name} leaves the social media.",
                        )
                    )
                    self.round_msg.append(
                        Message(
                    ...
                    if agent2.event.action_type == "shopping":
                        ...
                        self.logger.info(
                            f"{name} wants to chat with {agent_name2}, but {agent_name2} is shopping. So {name} does nothing."
                        )
                        message.append(
                            Message(
                                agent_id=agent_id,
                                action="LEAVE",
                                content=f"{name} wants to chat with {agent_name2}, but {agent_name2} is shopping. So {name} does nothing.",
                            )
                        )
                        self.round_msg.append(
                            Message(
            ...
            self.logger.info(f"{contacts} watched this webcast.")
        else:
            self.logger.info(f"{name} does nothing.")
            message.append(
                Message(
    ...
    def create_agent(self, i, api_key) -> RecAgent:
        ...
        agent = RecAgent(
            ...
            watching_url=utils.get_avatar_url(
                id=i, gender=self.data.users[i]["gender"], type="shopping"
            ),
    ...
    def create_user_role(self, id, api_key):
        ...
        watching_url = utils.get_avatar_url(
            id=id, gender=gender, type="shopping", role=True
        )
        chatting_url = utils.get_avatar_url(
